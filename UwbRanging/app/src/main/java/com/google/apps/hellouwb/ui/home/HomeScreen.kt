...
fun HomeScreen(uiState: HomeUiState, modifier: Modifier = Modifier) {
  ...
  Scaffold(
    ...
    Column(modifier = Modifier.padding(innerPadding)) {
      Row(modifier = Modifier.padding(innerPadding)) {   // 연결된 기기의 이름들
        ConnectStatusBar(
          uiState.connectedEndpoints.map { it.endpoint },
          uiState.disconnectedEndpoints
        )
      }
      Row { RangingPlot(uiState.connectedEndpoints) }    // 연결 기기의 상대 위치 정보
    }
  }
}
...
fun HomeTopAppBar(
  ...
  CenterAlignedTopAppBar(
    title = { Text("Juhwan's JoB") },
    actions = {
      val icon = if (isRanging) Icons.Filled.NearMe else Icons.Filled.NearMeDisabled   // 왼쪽 상단의 현 상태 표시
    ...
    connectedEndpoints.forEachIndexed { index, endpoint ->
      endpoint.position.distance?.let { distance ->
        endpoint.position.azimuth?.let { azimuth ->
          drawPosition(
            distance.value,
            azimuth.value,  // 0~360의 각도
            scale = scale,  // 0~20의 거리 (meter)
            centerOffset = center,  // 좌표
            color = ENDPOINT_COLORS[index % ENDPOINT_COLORS.size]
          )
          // Text("(${distance.value}, ${azimuth.value})") // text를 넣고 싶어....
        }
      }
    }
  }
...
//  }
  ...
  // 화면의 좌표를 알려줄 기저
  val angles = floatArrayOf(0f, 45f, 90f, 135f)  // 짜피 180으로 나누니깐
  angles.forEach {
    val rad = it * PI / 180
    val start =
...
private fun DrawScope.drawPosition(    // 위치 표시
  distance: Float,
  azimuth: Float,
  scale: Float,
  centerOffset: Offset,
  ...
  drawCircle(
    ...
    color = color,
    radius = 15.0f  // 그려지는 원의 반지름
  );
  drawLine(
    color = color,
    start = centerOffset,
    end = centerOffset.plus(Offset(x * scale, -y * scale)),
    strokeWidth = 2f
  );
}
...
fun ConnectStatusBar(
  ...
  Box(modifier.height(50.dp)) {
    Column {
      Text(text = "연결된 기기", color = Color.Black, fontWeight = MaterialTheme.typography.titleLarge.fontWeight)
      //
      Row {
        connectedEndpoints.forEachIndexed { index, endpoint ->
          Text(
        ...
        override val connectedEndpoints =
          listOf(
            ConnectedEndpoint(
              ...
              RangingPosition(
                distance = RangingMeasurement(5.0f),
            ...
            ConnectedEndpoint(
              UwbEndpoint("EP3", byteArrayOf()),
              RangingPosition(
                distance = RangingMeasurement(8.0f),
                azimuth = RangingMeasurement(-135.0f),
                elevation = null,
                elapsedRealtimeNanos = 200L
              ),
            ),
